...
.header{background:#1e293b;padding:10px 20px;border-bottom:1px solid #334155;display:flex;justify-content:space-between;align-items:center}
...
.card h2{font-size:1rem;margin-bottom:10px;color:#38bdf8;border-bottom:1px solid #334155;padding-bottom:6px}
.add-form{display:flex;gap:6px}
.add-form input{flex:1;padding:8px 10px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;font-size:.9rem}
.add-form input:focus{border-color:#38bdf8;outline:none}
.btn{padding:8px 14px;background:#38bdf8;color:#0f172a;border:none;border-radius:6px;font-weight:600;cursor:pointer;white-space:nowrap;font-size:.85rem}
.btn:hover{opacity:.85}.btn-sm{padding:4px 10px;font-size:.75rem}.btn-danger{background:#ef4444;color:#fff}
.btn-outline{background:transparent;border:1px solid #334155;color:#94a3b8}
.status-up{color:#6ee7b7}.status-down{color:#fca5a5}.status-warn{color:#fde047}
table{width:100%;border-collapse:collapse;font-size:.85rem}
th,td{padding:8px 10px;text-align:left;border-bottom:1px solid #334155}
th{color:#94a3b8;font-size:.75rem}.mono{font-family:monospace;font-size:.8rem}
tr:hover{background:#0f172a}
.auth-box{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);justify-content:center;align-items:center;z-index:1000}
.auth-card{background:#1e293b;border-radius:12px;padding:24px;width:100%;max-width:360px}
.auth-card h2{color:#38bdf8;margin-bottom:16px}
.auth-card input{width:100%;padding:10px;margin-bottom:10px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0}
.auth-card input:focus{border-color:#38bdf8;outline:none}
#loginError{color:#fca5a5;font-size:.8rem;display:none}
</style>
</head>
<body>
<div class="header"><h1>API Monitor</h1><div style="display:flex;gap:8px;align-items:center"><span id="userInfo" style="color:#94a3b8;font-size:.8rem;margin-right:8px"></span><button class="btn btn-sm btn-outline" onclick="showLogin()" id="loginBtn">Login</button><button class="btn btn-sm btn-outline" onclick="logout()" id="logoutBtn" style="display:none">Logout</button></div></div>
...
<button class="btn" style="width:100%;margin-top:4px" onclick="doLoginReal()">Login</button>
<button class="btn btn-outline" style="width:100%;margin-top:8px" onclick="hideLogin()">Cancel</button>
</div></div>
<div class="container">
<div class="stats">
...
<input id="monName" placeholder="Name" style="max-width:150px">
<button class="btn" onclick="addMonitor()">Add</button>
</div>
</div>
<div class="card">
<h2>Monitors <span style="font-size:.75rem;color:#94a3b8" id="planInfo"></span></h2>
<table><thead><tr><th>Name</th><th>URL</th><th>Status</th><th>Avg</th><th>SSL</th><th>Last</th><th></th></tr></thead>
...
<input id="newUserEmail" placeholder="User email" type="email">
<select id="newUserPlan" style="background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:8px"><option value="free">Free</option><option value="pro">Pro</option><option value="unlimited">Unlimited</option></select>
<button class="btn" onclick="adminCreateUser()">Create User</button>
</div>
<table><thead><tr><th>Email</th><th>Plan</th><th>Admin</th><th>Active</th><th>Last Login</th><th></th></tr></thead>
<tbody id="adminUsers"></tbody></table>
</div>
</div>
<script>
const API='';let TOKEN=localStorage.getItem('apitoken')||'';
...
if(!TOKEN){document.getElementById('monCount').textContent='login';return}
const r=await fetch(API+'/api/auth/me',{headers:authHeaders()});
if(r.status===401){TOKEN='';localStorage.removeItem('apitoken');updateUI();return}
const me=await r.json();
document.getElementById('userInfo').textContent=me.email+' ('+me.plan+')';
document.getElementById('planInfo').textContent='('+me.monitors_used+'/'+me.monitors_limit+' used)';
document.getElementById('logoutBtn').style.display='inline-block';
document.getElementById('loginBtn').style.display='none';
if(me.is_admin) document.getElementById('adminCard').style.display='block';
const sr=await fetch(API+'/api/stats',{headers:authHeaders()});
if(sr.status===401)return;
...
document.getElementById('downCount').textContent=d.length-up;
const avgs=d.filter(m=>m.avg_ms).map(m=>m.avg_ms);
document.getElementById('avgAll').textContent=avgs.length?Math.round(avgs.reduce((a,b)=>a+b)/avgs.length)+'ms':'--';
const tbody=document.getElementById('monitors');
tbody.innerHTML=d.map(m=>{
let s=m.last_status?m.last_status<400?'<span class="status-up">UP</span>':'<span class="status-down">'+m.last_status+'</span>':'<span class="status-down">DOWN</span>';
...
return '<tr onclick="showDetail('+m.id+',\''+(m.name||m.url).replace(/'/g,'&#39;')+'\')" style="cursor:pointer"><td>'+(m.name||'-')+'</td><td class="mono">'+m.url.substring(0,45)+'</td><td>'+s+'</td><td>'+(m.avg_ms||'--')+'ms</td><td>'+ssl+'</td><td class="mono">'+(m.last_checked||'').substring(11,19)+'</td><td><button class="btn btn-sm btn-danger" onclick="event.stopPropagation();delMon('+m.id+')">X</button></td></tr>';}).join('');
if(me.is_admin) loadAdmin();
}
async function loadAdmin(){
const r=await fetch(API+'/api/admin/users',{headers:authHeaders()});
const users=await r.json();
document.getElementById('adminUsers').innerHTML=users.map(u=>'<tr><td>'+u.email+'</td><td>'+u.plan+'</td><td>'+u.is_admin+'</td><td>'+u.is_active+'</td><td class="mono">'+(u.last_login||'').substring(0,16)+'</td><td><button class="btn btn-sm btn-danger" onclick="adminDeleteUser(\''+u.id+'\')">X</button></td></tr>').join('');
}
...
async function loadChecks(){const r=await fetch(API+'/api/monitors/'+window._mid+'/checks?limit=30',{headers:authHeaders()});const d=await r.json();
document.getElementById('checks').innerHTML=d.map(c=>'<tr><td>'+c.status+'</td><td>'+(c.ms||'--')+'ms</td><td>'+(c.ssl_days!=null?c.ssl_days+'d':'--')+'</td><td style="color:#fca5a5;font-size:.75rem">'+(c.error||'').substring(0,50)+'</td><td class="mono">'+(c.at||'').substring(11,19)+'</td></tr>').join('');}
async function addMonitor(){const url=document.getElementById('monUrl').value;const name=document.getElementById('monName').value;await fetch(API+'/api/monitors',{method:'POST',headers:authHeaders(),body:JSON.stringify({url,name})});document.getElementById('monUrl').value='';document.getElementById('monName').value='';loadStats();}
async function delMon(id){if(confirm('Delete?')){await fetch(API+'/api/monitors/'+id,{method:'DELETE',headers:authHeaders()});loadStats();}}
function showLogin(){document.getElementById('authBox').style.display='flex';}
function hideLogin(){document.getElementById('authBox').style.display='none';}
async function doLoginReal(){const email=document.getElementById('loginEmail').value;const pw=document.getElementById('loginPass').value;
...
if(r.ok){TOKEN=d.token;localStorage.setItem('apitoken',TOKEN);hideLogin();updateUI();}else{document.getElementById('loginError').style.display='block';document.getElementById('loginError').textContent=d.detail||'Login failed';}}
function logout(){TOKEN='';localStorage.removeItem('apitoken');updateUI();}
async function adminCreateUser(){const email=document.getElementById('newUserEmail').value;const plan=document.getElementById('newUserPlan').value;
const r=await fetch(API+'/api/admin/users',{method:'POST',headers:authHeaders(),body:JSON.stringify({email,plan})});const d=await r.json();alert(d.temp_password?'Created! Temp PW: '+d.temp_password:JSON.stringify(d));loadAdmin();}
async function adminDeleteUser(id){if(confirm('Delete user?')){await fetch(API+'/api/admin/users/'+id,{method:'DELETE',headers:authHeaders()});loadAdmin();}}
function updateUI(){if(TOKEN)loadStats();else{document.getElementById('monCount').textContent='login';document.getElementById('upCount').textContent='-';document.getElementById('downCount').textContent='-';document.getElementById('avgAll').textContent='-';document.getElementById('monitors').innerHTML='';document.getElementById('logoutBtn').style.display='none';document.getElementById('loginBtn').style.display='inline-block';document.getElementById('adminCard').style.display='none';}}
setInterval(()=>{if(TOKEN)loadStats();if(window._mid)loadChecks()},15000);
document.getElementById('loginPass').addEventListener('keydown',(e)=>{if(e.key==='Enter')doLoginReal();});
updateUI();
</script>
</body>
</html>""")
